...
static int HilinkWifiReboot(void)
{
    hi_soft_reboot(HI_SYS_REBOOT_CAUSE_USR_NORMAL_REBOOT);
    return 0;
}
...
hi_void app_main(hi_void)
{
    ...
    app_demo_upg_init();
#ifdef CONFIG_HILINK
    HILINK_SetNetConfigMode(HILINK_NETCONFIG_NAN_SOFTAP);	//HILINK_NETCONFIG_WIFI

    extern int HILINK_GetDevSurfacePower_Def(char *power);

    HILINK_RegDevSurfacePowerCallback(HILINK_GetDevSurfacePower_Def);
    ...
    /* 硬重启目前使用厂商接口 */
    sdkAttr->rebootHardware = HilinkWifiReboot;
    hilink_printf("[HiLink SDK] task stack size: monitor[%d], main[%d], otaCheck[%d], otaUpdate[%d].\r\n",
            sdkAttr->monitorTaskStackSize, sdkAttr->deviceMainTaskStackSize, sdkAttr->otaCheckTaskStackSize, sdkAttr->otaUpdateTaskStackSize);
