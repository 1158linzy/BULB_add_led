...
typedef enum {
    ...
    HI_WIFI_MGMT_FRAME_PROTECTION_BUTT
} hi_wifi_pmf_options;
 ...
 * @ingroup hi_wifi_basic
 *
 * rate of packet.CNcomment:报文的速率信息联合体，包含11n和11a/b/g模式的常用定义.CNend
 *
 */
typedef union {
    struct {
        unsigned char   ht_mcs        : 6;   /* 11n rate code. CNcomment: 11n模式速率码值,0x20～0x27，对应速率MCS0～7.CNend */
        unsigned char   protocol_mode : 2;   /* protocol mode 0:11b 1:11a/g 2:11n. CNcomment: 协议模式,0:11b 1:11a/g 2:11n.CNend */
    } ht_rate;
    struct {
        unsigned char   bit_legacy_rate   : 4; /* 11a/b/g rate code. CNcomment: 11a/b/g模式速率码值,例如0对应1M,0x1B对应6M,0x1A对应12M.CNend */
        unsigned char   reserved1     : 2;  /* reserved. CNcomment: 保留值，一般为0.CNend */
        unsigned char   protocol_mode : 2;  /* protocol mode 0:11b 1:11a/g 2:11n. CNcomment: 协议模式,0:11b 1:11a/g 2:11n.CNend */
    } st_legacy_rate;
} hi_wifi_rate_union;
 ...
 * @ingroup hi_wifi_basic
 *
 * information of softap's user.CNcomment:与softap相连的station mac地址,最新接收报文的rssi,速率信息.CNend
 *
 */
typedef struct {
    unsigned char   mac[HI_WIFI_MAC_LEN];     /* MAC address.CNcomment:与softap相连的station mac地址.CNend */
    char            rssi;                     /* rssi.CNcomment:softap上一次接收相连的station报文的rssi.CNend */
    hi_wifi_rate_union rate;                  /* data rate code.CNcomment:softap上一次接收相连的station报文速率码值，数据报文为非0值CNend */
} hi_wifi_ap_sta_info;
...
typedef struct {
    ...
    unsigned short mgmt_len;
    unsigned char *mgmt_buff;
} hi_wifi_scan_result_stru;
 ...
 * @ingroup hi_wifi_basic
 *
 * callback function definition of scan result report.CNcommment:扫描结果上报回调函数.CNend
 * @param  scan_result [IN] TYPE #hi_wifi_scan_result_stru *, scan result
 * @attention scan_result为空时，表示扫描结束
 */
typedef void (*hi_wifi_scan_no_save_cb)(hi_wifi_scan_result_stru *scan_result);
...
*
* @attention  The rssi value is derived from latest frame received,127 is invalid.\n
              CNcomment:该值从最新收到的来自所连接AP的帧获取,127代表无效值.CNend
* @param  NULL
*
...
/**
* @ingroup  hi_wifi_basic
* @brief  Set offset of extern tsf.CNcomment: 设置STA接收beacon外置提前量.CNend
*
* @par Description:
*         Set offset of extern tsf.CNcomment: 设置STA接收beacon外置提前量.CNend
*
* @attention  make sure STA has connected an AP and in powersave mode
              CNcomment:保证STA连接上路由器并且开启低功耗开关之后调用本函数.CNend
* @param  ext_tbtt_offset    [IN]  Type  #unsigned short offset of extern tsf,suggested range[6000,9000]
          CNcomment:外置时钟接收beacon的提前量，配置为0则采用当前值/默认值,单位us,建议合理范围6000-9000us.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
* @par Dependency:
...
* @see  NULL
*/
int hi_wifi_sta_set_ext_offset(unsigned short ext_tbtt_offset);
...
*                 HI_WIFI_SECURITY_SAE.CNend \n
*                 HI_WIFI_SECURITY_WPA3_WPA2_PSK_MIX.CNend \n
...
*             10. The PMF of WPA3 is REQUIRE, and the PMF of WPA2/WPA3 is OPTIONAL.CNcomment:10．WPA3的PMF为强制，WPA2/WPA3的PMF为可选.CNend \n
*             11. The PMKID connection is supported. The PMKID is stored in the memory for 12 hours..CNcomment:11．支持PMKID连接，PMKID保存时间为12小时CNend
...
*
* @attention  1.sta_list: malloc by user.CNcomment:1.查询结果参数。由用户动态申请。CNend \n
...
*               CNcomment:5.只获取关联成功的sta。CNend
*             6.This api do not support invoking by multi-threads at the same time,and will block invoker until return.
*               CNcomment:6.本API不可重入，多线程同时调用需注意互斥,接口为同步接口，会阻塞调用者，不建议在定时器或中断中使用CNend
...
/**
* @ingroup  hi_wifi_basic
* @brief  set mac address for AP.CNcomment:设置用于AP的MAC地址.CNend
*
* @par Description:
*           Set mac address for AP.CNcomment:设置用于AP的mac地址.CNend
*
* @attention  1.invoke before creating any STA or AP.CNcomment:在系统初始化完成并且STA和AP都还未创建时调用。CNend
* @param  mac_addr          [IN]     Type #char *, mac address.CNcomment:MAC地址.CNend
* @param  mac_len           [IN]     Type #unsigned char, mac address length.CNcomment:MAC地址长度.CNend
*
* @retval #HISI_OK  Excute successfully
...
* @see  NULL
*/
int hi_wifi_set_ap_macaddr(const char *mac_addr, unsigned char mac_len);
...
/**
* @ingroup  hi_wifi_basic
* @brief  get mac address of AP.CNcomment:获取用于AP的MAC地址.CNend
*
* @par Description:
*           Get mac address of AP.CNcomment:获取用于AP的mac地址.CNend
*
* @attention  NULL
* @param  mac_addr          [OUT]    Type #char *, mac address.
* @param  mac_len           [IN]     Type #unsigned char, mac address length.
...
* @see  NULL
*/
int hi_wifi_get_ap_macaddr(char *mac_addr, unsigned char mac_len);

/**
* @ingroup  hi_wifi_basic
* @brief  get rx fcs info.CNcomment:获取常收模式下接收到的帧数.CNend
*
* @attention  1. be sure STA has been started before using this API. CNcoment: 调用该API前，确保STA已起来.CNend
              2. API is invalid only in always rx mode. CNcoment: 此API仅在常收模式下生效.CNend
*
* @param  rx_pkt_num    [OUT]    Type #unsigned int *, receive package num.
*
* @retval #HISI_OK  Excute successfully
* @retval #Other    Error code
* @par Dependency:
...
* @see  NULL
*/
int hi_wifi_get_rx_info(unsigned int *rx_pkt_num);
...
*
* @attention  1.May influence evm and flatness of power per channel.
*               CNcomment:1.offset设置,可能会影响信道功率平坦度和evm.CNend
*             2.Offset is not 1:1 for tx power.
*               CNcomment:2.Offset和发送功率不是严格1:1的关系.CNend
* @param  ifname          [IN]     Type #char *, device name. CNcomment:接口名.CNend
* @param  offset          [IN]     Type #signed short, offset,-250~30,unit:0.1dB. CNcomment:偏移值,-250~30，单位0.1dB.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
*
...
*
* @attention  NULL
* @param  iftype        [IN]   Type  #hi_wifi_iftype, interface type,usually use HI_WIFI_IFTYPE_STATION or
                                      HI_WIFI_IFTYPE_AP. CNcomment:接口类型,一般只采用HI_WIFI_IFTYPE_STATION或者
                                      HI_WIFI_IFTYPE_AP。CNend
* @param  fram_type     [IN]   Type  #hi_wifi_frame_type, frame type，HI_WIFI_IFTYPE_STATION iftype only supports
                                      Probe Request. CNcomment:帧类型, HI_WIFI_IFTYPE_STATION 接口类型只支持
                                      Probe Request帧。CNend
...
*
* @attention  1.tpc_value set to 0, disbale it.
*               set to 1, enable auto power control, increase 3dB transmit power.
*               set to 2, enable auto power control,
*               but the 3dB transmission power is not increased.
*             CNcomment:1.tpc_value范围是0~2,0:关闭发送功率自动控制,
*              1:打开发送功率自动控制,需要降低功率场景降低功率,需要提高功率场景提高发射功率.
*                一般提高3dB功率，当提高3dB超过芯片最大功率值时，提高到最大功率门限.
*              2:打开发送功率自动控制,需要降低功率场景降低功率,其他场景不提高发射功率.
*             目前TPC的默认模式为1,如果需要切换其他TPC模式,需在设备处于未关联状态时设置.CNend
* @param  ifname          [IN]     Type #char *, device name. CNcomment:接口名.CNend
* @param  ifname_len      [IN]     Type #unsigned char, interface name length.CNcomment:接口名长度.CNend
...
*             2.Channel set to 0 indicates no channel specified,otherwise STA will scan on the specified
                channel firstly.\n CNcomment:2.不指定信道时channel参数可设置为0,如果指定信道则优先在指定信道扫描，
                指定信道扫描不到则全信道扫描.CNend
*             3.When start anchor function, it will report beacon.CNcomment:3.
                调用了开始锚点的函数则会开始上报对应AP的Beacon帧.CNend
*             4.When anchor AP parameter changed and need to restart anchor , it should restart STA and then restart
...
* @param  cb           [IN]     Type #hi_wifi_anchor_cb, callback function.CNcomment:用户实现供驱动调用的接收
                                      beacon处理函数.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other    Error code
...
/**
* @ingroup  hi_wifi_basic
* @brief  customer scan interface.CNcomment:定制化扫描接口.CNend
*
* @par Description:
*           scan reslut report callback.CNcomment:扫描结果上报.CNend
*
* @attention  1. scan result should be copied to user space as soon as possible.
              2. scan reslut can not be freed by user.
*             CNcomment:
              1. 不能在回调函数中做复杂的处理，回调函数需尽快返回
              2. 用户不可对扫描结果做释放操作.
              3. 回调函数中需要尽快拷贝上报的内存；
...
* @see  NULL
*/
int hi_wifi_raw_scan(const char *ifname,
                         hi_wifi_scan_params *custom_scan_param, hi_wifi_scan_no_save_cb cb);

/**
* @ingroup  hi_wifi_basic
* @brief  set phy txrate.CNcomment:设置 phy 发送速率.CNend
*
* @attention  1. must be called under no power save mode;
              2. auto_rate must be disable when set fix rate.
*             CNcomment:
              1. 只能在非低功耗模式下调用该API;
              2. 固定速率设置的前提是TRC为去使能状态.
...
* @see  NULL
*/
int hi_wifi_set_phy_txrate(const char *ifname, unsigned char auto_rate, alg_param_stru *alg_param);

/**
* @ingroup  hi_wifi_basic
* @brief  Enable anti microwave oven interference mode or not.CNcomment:是否使能抗微波炉干扰模式.CNend
*
* @par Description:
*           Enable anti microwave oven interference mode or not.CNcomment:是否使能抗微波炉干扰模式.CNend
*
* @param  enable         [IN]     Type #unsigned char, 0:disable, 1:enable, others:invalid.
*                                 CNcomment:是否使能抗微波炉干扰模式,0:禁止,1:使能,其它值:无效.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
*
...
* @see  NULL
*/
int hi_wifi_enable_anti_microwave_intrf(unsigned char enable);

/**
* @ingroup  hi_wifi_basic
* @brief  Enable anti-interference mode or not.CNcomment:是否使能抗干扰模式.CNend
*
* @par Description:
...
* @see  NULL
*/
int hi_wifi_enable_intrf_mode(const char* ifname, unsigned char enable, unsigned short flag);

/**
* @ingroup  hi_wifi_basic
* @brief  Set hardware sent count in 11b mode.CNcomment:设置11b模式各档速率的硬件发送次数.CNend
*
* @par Description:
...
* @see  NULL
*/
int hi_wifi_set_11b_tx_count(const char *ifname, unsigned char cnt0, unsigned char cnt1, unsigned char cnt2, unsigned char cnt3);
