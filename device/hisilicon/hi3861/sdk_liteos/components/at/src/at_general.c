...
{
    if (os_shell_netstat(argc, (const char **)argv) == LOS_OK) {
        return HI_ERR_SUCCESS;
    }
...
static hi_u32 at_flash_write(hi_s32 argc, const hi_char **argv)
{
    if ((at_param_null_check(argc, argv) == HI_ERR_FAILURE) || (argc != 1) ||
        ((integer_check(argv[0]) == HI_ERR_FAILURE))) {
        return HI_ERR_FAILURE;
    }

    if ((atoi(argv[0]) <= 0) || (atoi(argv[0]) > AT_DATA_MAX_LEN)) {
        return HI_ERR_FAILURE;
    }

    /* enter data recving mode */
    g_at_ctrl.send_len = (hi_u16)atoi(argv[0]);
    g_at_ctrl.at_state = AT_DATA_RECVING;
...
#ifdef CONFIG_FACTORY_TEST_MODE
    {"+WRITE", 6, HI_NULL, HI_NULL, (at_call_back_func)at_flash_write, HI_NULL},
#endif
