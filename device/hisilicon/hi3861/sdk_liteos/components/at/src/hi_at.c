...
#define AT_DEFAULT_PROC_TASK_SIZE   0x1000 /* 0x800:softap start would fail. */
...
hi_void at_get_flash_write_data(hi_char c)
{
    static hi_u16 flash_write_data_index = 0;
    g_at_data[flash_write_data_index] = c;
    if ((flash_write_data_index >= (g_at_ctrl.send_len - 1)) || (flash_write_data_index >= (AT_DATA_MAX_LEN - 1))) {
        g_at_ctrl.at_state = AT_DATA_SENDING;
        g_at_ctrl.is_first_recv_data = HI_TRUE;
        g_at_ctrl.send_len = flash_write_data_index + 1;

        flash_write_data_index = 0;
        at_notify();
        hi_free(HI_MOD_ID_APP_AT, g_at_data);
        g_at_data = HI_NULL;
    }
    flash_write_data_index++;
}
...
hi_void at_parse_uart_char(hi_char ch)
{
    switch (g_at_ctrl.at_state) {
        ...
        case AT_DATA_RECVING:
            ...
            at_get_send_data(ch); /* in non_factory mode, data send to tcp/udp, user can add other branch here. */
#else
            at_get_flash_write_data(ch); /* in factory mode, data write to flash, user can add other branch here. */
#endif
            break;
...
hi_void at_write_flash_data(hi_char *data, hi_u32 data_len)
{
    /* implementation sample: write data to user partition table. */
    hi_u32 write_partition_addr = 0;
    hi_u32 write_partition_size = 0;
    hi_u32 ret = hi_get_usr_partition_table(&write_partition_addr, &write_partition_size);
    if (ret != HI_ERR_SUCCESS) {
        return;
    }

    if (data_len > write_partition_size) {
        hi_at_printf("no enough space for data write\r\n");
        return;
    }

    /* user can change flash write addr here. */
    ret = hi_flash_write(write_partition_addr, data_len, (const hi_u8 *)data, HI_TRUE);
    if (ret == HI_ERR_SUCCESS) {
        hi_at_printf("write to flash:0x%x finish, writed len:%d\r\n", write_partition_addr, data_len);
    } else {
        hi_at_printf("write to flash fail:0x%x\r\n", ret);
    }
}
...
#ifndef CONFIG_FACTORY_TEST_MODE
                /* in non_factory mode, data send to tcp/udp, user can add other branch here. */
                at_send_serial_data(buf);
#else
                /* in factory mode, data write to flash, user can add other branch here. */
                at_write_flash_data(buf, g_at_ctrl.send_len);
#endif
                g_at_ctrl.at_state = AT_IDLE;
