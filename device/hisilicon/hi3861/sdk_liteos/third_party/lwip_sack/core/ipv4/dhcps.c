  ...
  while ((q != NULL) && (offset < offset_max) && (options[offset] != DHCP_OPTION_END)) {
    ...
    switch (op) {
      ...
      case (DHCP_OPTION_SUBNET_MASK):
        LWIP_ERROR("len == sizeof(u32_t)", len == sizeof(u32_t), return ERR_VAL);
      ...
      case (DHCP_OPTION_DNS_SERVER):
        ...
        LWIP_ERROR("len %% sizeof(u32_t) == 0", len % sizeof(u32_t) == 0, return ERR_VAL);
...
decode_next:
      ...
      if (!dhcps_option_given(dhcp, decode_idx)) {
        ...
        if (decode_len > sizeof(u32_t)) {
          ...
          LWIP_ERROR("decode_len %% sizeof(u32_t) == 0", decode_len % sizeof(u32_t) == 0, return ERR_VAL);
          dhcps_got_option(NULL, decode_idx);
